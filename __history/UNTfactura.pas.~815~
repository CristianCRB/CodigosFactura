...
uses
  ...
  Vcl.Menus, Vcl.StdCtrls, cxButtons, cxTextEdit, cxLabel,UNTconn,UntSplash,
  {unidades a incluir}
...
procedure TFrmFactura.BtnCargarFacturasClick(Sender: TObject);
var
...
procedure TFrmFactura.BtnTransmitirClick(Sender: TObject);
begin
...
//CODIFICACION PDF//
function TFrmFactura.EncodeFile(const PDF: string): AnsiString;
  var
...
Function TFrmFactura.EnvioPDF(pUsuario,pPassword,pUrl,pCufe,pToken:String;pPDFEncode:WideString):AnsiString;
    var
...
procedure TFrmFactura.cxGrid1DBTableView1CellDblClick(
...
procedure TFrmFactura.cxGrid1DBTableView1MarcadoCustomDrawCell(
  Sender: TcxCustomGridTableView; ACanvas: TcxCanvas;
  AViewInfo: TcxGridTableDataCellViewInfo; var ADone: Boolean);
  var
   vDibujo :TBitmap;
...
//OBTENER CUFE//
function TFrmFactura.EnvioXMLObtenerCufe(pUsuario, pPassword, pUrl, pToken: String;
  ...
  begin
    ...
    vIdHTTF1.Request.CustomHeaders.Values['Authorization']:=pToken;
    try
      FrmSplash.LblActualizando.Caption := 'Obteniendo CUFE';
      FrmSplash.ShowModal;
      htmlDna := vIdHTTF1.Post(pUrl, vMemory);
      Sleep(5000);
    ...
    Result:= vCufe;
    FrmSplash.LblActualizando.Caption := 'CUFE Obtenido';
    FrmSplash.ShowModal;
    FreeAndNil(vMemory);
    FreeAndNil(vSSL2);
    FreeAndNil(vIdHTTF1);
...
//OBTENER TOKEN//
Function TFrmFactura.ObtenerToken(pUsuario,pPassword,pUrl:String):AnsiString;
    var
      vIdHTTF1:TIdHTTP;
      vSSL2: TIdSSLIOHandlerSocketOpenSSL;
    ...
    begin
      ...
      begin
        ...
        try
          FrmSplash.LblActualizando.Caption := 'Obteniendo Token';
          FrmSplash.ShowModal;
      ...
      Result:= vToken;
      FrmSplash.LblActualizando.Caption := 'Token Obtenido';
      FrmSplash.ShowModal;
      FreeAndNil(vMemory);
      FreeAndNil(vSSL2);
      FreeAndNil(vIdHTTF1);
...
//GENERAR XML//
procedure TFrmFactura.GenerarXML(pDocumento: String);
   type tNombre = Record
      vNombre1  :String;
      vNombre2  :String;
      vApellido1:String;
...
begin
  ...
  FrmSplash.LblActualizando.Caption := 'Creando XML';
  FrmSplash.ShowModal;
    with CodigoXML do begin
      ...
      Add('        <subtotal>'+FormatFloat('0.00',QryMaestroFacturasSUBTOTAL.AsFloat)+'</subtotal>');
      ...
      if QryDetallesFacturaIDENTIFICACIONPRODUCTOS.AsInteger <> 0 then
        begin
          ...
          Add(vCaracterTab+vCaracterTab+vCaracterTab+'<impuestolinea>'+FormatFloat('0.00',QryDetallesFacturaIMPUESTOLINEA.AsFloat)+'</impuestolinea>');
          ...
          Add(vCaracterTab+vCaracterTab+vCaracterTab+'<baseimpuestos>'+FormatFloat('0.00',QryDetallesFacturaBASEIMPUESTOS.AsFloat)+'</baseimpuestos>');
          ...
          Add(vCaracterTab+vCaracterTab+vCaracterTab+'<precioUnitario>'+FormatFloat('0.00',QryDetallesFacturaPRECIOUNITARIO.AsFloat)+'</precioUnitario>');
          Add(vCaracterTab+vCaracterTab+vCaracterTab+'<importe>'+FormatFloat('0.00',QryDetallesFacturaIMPORTEDETALLE.AsFloat)+'</importe>');
          ...
          Add(vCaracterTab+vCaracterTab+vCaracterTab+'<importe>'+FormatFloat('0.00',QryDetallesFacturaIMPORTEIMPUESTO.AsFloat)+'</importe>');
          Add('  </Imp>');
          Add('</Impuestos>');
        end;
    ...
    stream := TMemoryStream.Create;
    FrmSplash.LblActualizando.Caption := 'Guardando XML';
    FrmSplash.ShowModal;
    ...
    begin
      ...
      begin
         ...
         FrmSplash.LblActualizando.Caption := 'Enviando Archivo' +DBLookupComboBox1.Text+QryMaestroFacturasFOLIO.AsString;
         FrmSplash.ShowModal;
         ...
         FrmSplash.LblActualizando.Caption := 'Documento '+pDocumento+' enviado con éxito';
         FrmSplash.LblMensajeCufe.Caption := vRegactualiza.cufe;
         FrmSplash.ShowModal;
...
procedure TFrmFactura.ActualizaEstado(pDocumento,pXML,pCufe,pError:String);
begin
