...
uses UntFrmDatosEmpresa,UNTfactura,UNTfacturaexp,UNTnotacredito,UNTconn,
  UNTfacturausada, UNTbasededatos, UNTestadisticasfact;
...
begin
  ...
  if DMconn.QryDatosEmpresarango.AsInteger <> DMconn.QryServidor2rango.AsInteger then
  begin
    ShowMessage('No tienes rango de numeración para facturar');
  end;
end;
...
procedure TForMenuPrincipal.dxTileControl1Item1Click(
...
begin
  ...
  begin
    if Sender.DetailOptions.DetailControl = nil then
    begin
      Sender.DetailOptions.DetailControl := TFrmEstadisticasFact.Create(Self);
      Sender.DetailOptions.DetailControl.Name := Sender.Name +'Details';
      TFrmEstadisticasFact(Sender.DetailOptions.DetailControl).CdtsCountDetaTransaccion.CreateDataSet;
      TFrmEstadisticasFact(Sender.DetailOptions.DetailControl).CdtsCountDetaTransaccion.Open;
      TFrmEstadisticasFact(Sender.DetailOptions.DetailControl).QryCountDetaTransaccion.Close;
      TFrmEstadisticasFact(Sender.DetailOptions.DetailControl).QryCountDetaTransaccion.Open;
    end;
  end;
  if DMconn.QryDatosEmpresarango.AsInteger <> DMconn.QryServidor2rango.AsInteger then
  begin
